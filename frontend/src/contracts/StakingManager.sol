...
contract StakingManager is Ownable{
    ...
    struct Pool {
        ...
        uint256 weighting; // weighting of rewards per block
    }

    Pool[] public pools; // Staking pools

    // Total weighting of pools
    uint256 totalWeighting;
     ...
     */
    function createPool(IERC20 _stakeToken, uint256 _weighting) external onlyOwner {
        Pool memory pool;
        pool.stakeToken = _stakeToken;
        pool.weighting = _weighting;
        pools.push(pool);
        uint256 poolId = pools.length - 1;
        totalWeighting += _weighting;
    ...
    function updatePoolRewards(uint256 _poolId) private {
        ...
        uint256 rewards = blocksSinceLastReward * getPoolRewardsPerBlock(_poolId);
        pool.accumulatedRewardsPerShare += (rewards * REWARDS_PRECISION / pool.tokensStaked);
        pool.lastRewardedBlock = block.number;
    }

    /**
     * @dev Get pool ratio of rewards
     */
    function getPoolRewardsPerBlock(uint256 _poolId) private view returns(uint256) {

        return rewardTokensPerBlock * pools[_poolId].weighting / totalWeighting;
    }
    
    /**
     * @dev Get user total rewards
     */
    function getTotalRewards(uint256 _poolId) public view returns(uint256) {
        Pool storage pool = pools[_poolId];
        ...
        if (pool.tokensStaked != 0) {
            ...
            uint256 blocksSinceLastReward = block.number - pool.lastRewardedBlock;
            uint256 rewards = blocksSinceLastReward * getPoolRewardsPerBlock(_poolId);
            uint256 accumulatedRewardsPerShare = pool.accumulatedRewardsPerShare + (rewards * REWARDS_PRECISION / pool.tokensStaked);
     ...
     * @dev Get user staked
     */
    function getStaked(uint256 _poolId, address _user) public view returns(uint256) {
        return poolStakers[_poolId][_user].amount;
    }
