...
async function deployLP(name: string, symbol: string, amount: string): Promise<LPFactory> {
  const lp_contract = await ethers.getContractFactory("LPFactory");
  const lp = await lp_contract.deploy(name,symbol,amount);
  await lp.deployed();
  return lp;
}
...
describe("StakingManager", function () {
  ...
  this.beforeEach(async () => {
    lp1 = await deployLP("LP1", "LP1", "1000000000000");
    lp2 = await deployLP("LP2", "LP2", "1000000000000");
